...
- **Person-name guard (people only) — allowlist-based (per user choice):**
  ...
  - **Codename exclusion:** org codenames that collide with common Hebrew given names (e.g., `ברק`, `דניאל`) have English equivalents in the glossary and **must be translated, not masked**. They are listed in `data/person_names/codenames.txt` (one Hebrew term per line, `#` comments allowed) and removed from the allowlist at load time — `load_person_names()` filters `first`/`last` against this set and also against any `term_he` that appears in `glossary.csv` with status `approved`/`keep_source` (so a codename added to the glossary is automatically excluded from PERSON masking even before the codenames file is updated). This prevents the guard from freezing a codename as `⟦PERSON_n⟧` while the glossary expects it translated.
  ...
  - Scope: people only — orgs/places translate normally (user confirmed). Codenames are treated as terms, not people.
  - Failure mode: rare names absent from 592/818 → caught by optional NER or surfaced as `unknown_terms` for human queue; corpus-specific uncommon names get added to allowlist on first review (one-time cost). Codenames accidentally masked → glossary_retention would fail (English expected but Hebrew PERSON marker preserved) and QA would quarantine; with the exclusion the term translates via glossary as intended.
